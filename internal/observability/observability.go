...
// MetricsService implements ports.MetricsService for Prometheus (mocked for MVP).
...
func (m *MetricsService) RecordCommandLatency(protocol domain.Protocol, status string, latency time.Duration) {
	// In production, this would call prometheus.HistogramVec.Observe()
...
func (m *MetricsService) RecordCommandDropped() {
	// In production, this would increment prometheus.CounterVec
...
func (m *MetricsService) Start() {
	...
	// Start basic HTTP server for metrics (In production, wire promhttp.Handler() here)
	mux := http.NewServeMux()
	mux.HandleFunc("/metrics", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte("# HELP gateway_commands_processed_total\n"))
	})
